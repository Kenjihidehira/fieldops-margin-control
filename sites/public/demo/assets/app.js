...
function render(payload) {
  ...
  qs("#projectBoard").innerHTML = visible.map((project) => `<article class="project-card ${project.risk}"><header><span>${project.id}</span><b>${label(project.risk)}</b></header><strong>${project.name}</strong><small>${project.segment} | ${project.customer}</small><div class="project-metrics"><div><span>${project.progress}%</span><small>progresso</small></div><div><span>${project.forecastMargin}%</span><small>margem</small></div><div><span>${project.healthScore}</span><small>pontuacao</small></div></div><div class="risk-meter"><i style="width:${riskLoad(project)}%"></i></div><footer><span>${money(project.marginAtRisk)}</span><small>margem em risco</small></footer></article>`).join("");
...
async function loadDashboard() { render(await requestState(`?${new URLSearchParams(filters)}`)); }
...
function showError(error) { qs("#automationLog").textContent = error.message; }
